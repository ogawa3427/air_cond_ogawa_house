...
// The USBSerial connection baud rate.
// i.e. Status message will be sent to the PC at this baud rate.
// Try to avoid slow speeds like 9600, as you will miss messages and
// cause other problems. 115200 (or faster) is recommended.
// NOTE: Make sure you set your USBSerial Monitor to the same speed.
...
// e.g. It will stop decoding a message and start sending it to USBSerial at
...
uint64_t light_on = 0x344A9034A4;
uint64_t light_off = 0x344A90F464;
...
void setup() {
  ...
  M5.Display.println("IRrecvDumpV2");
  M5.Display.endWrite();
...
void loop() {
  ...
  if (mode_toggle) {
    if (M5.BtnA.wasPressed()) {
      ...
      M5.Display.println("Send IR");
      M5.Display.endWrite();
      for (int i = 0; i < 5; i++) {
        uint64_t data = 0x344A9034A4;  // 送信するデータ
        irsend.sendPanasonic64(data);  // Panasonic プロトコルでデータを送信します。
        delay(40);
      }
    }
  }
  ...
  if (irrecv.decode(&results)) {
    ...
    USBSerial.println(resultToTimingInfo(&results));
